...
from openpi.models import nativemem_config
...
class NativeMEM(_model.BaseModel):
    def __init__(self, config: nativemem_config.NativeMEMConfig, rngs: nnx.Rngs):
    ...
    def compute_loss(
        ...
        prefix_tokens, prefix_mask, prefix_ar_mask, mem_offset, n_aux = self.embed_prefix(observation)
        suffix_tokens, suffix_mask, suffix_ar_mask, adarms_cond = self.embed_suffix(observation, x_t, time)

        # self / causal: joint forward
        ...
        action_loss = jnp.mean(jnp.square(v_t - u_t), axis=[1, 2])  # (B,)
