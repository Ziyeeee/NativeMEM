...
from openpi.models.nativemem_config import NativeMEMConfig
import openpi.models.siglip as _siglip
from openpi.shared import array_typing as at

logger = logging.getLogger("openpi_nativemem")
...
class NativeMEMRTC(_model.BaseModel):
    def __init__(self, config: NativeMEMConfig, rngs: nnx.Rngs):
        super().__init__(config.action_dim, config.action_horizon, config.max_token_len)
        self.pi05 = config.pi05
        self.dtype = config.dtype
        self.memory_insert_type = config.memory_insert_type
        self.max_delay = config.max_delay
        assert self.memory_insert_type in ["self", "causal"], f"Invalid memory_insert_type: {self.memory_insert_type}"
    ...
    def embed_prefix(
        ...
        """Embed prefix tokens. Returns (tokens, input_mask, ar_mask, mem_offset, n_aux_tokens).

        mem_offset: the token index in prefix where memory tokens start.
        n_aux_tokens: total number of memory tokens appended.
        """
        input_mask = []
        ar_mask = []
        tokens = []
        ...
        if obs.memory is not None:
            ...
            input_mask.append(jnp.ones((memory_tokens.shape[0], 1), dtype=jnp.bool_))
            ar_mask += [False]

            tokens.append(memory_tokens)
            input_mask.append(obs.memory_mask)
            if self.memory_insert_type == "causal":
                ar_mask += [True] + [False] * (memory_tokens.shape[1] - 1)
            else:  # "self"
                ar_mask += [False] * memory_tokens.shape[1]
            n_aux += 1 + memory_tokens.shape[1]

        tokens = jnp.concatenate(tokens, axis=1)
        input_mask = jnp.concatenate(input_mask, axis=1)
        ar_mask = jnp.array(ar_mask)
        return tokens, input_mask, ar_mask, mem_offset, n_aux
    ...
    def compute_loss(
        ...
        prefix_tokens, prefix_mask_attn, prefix_ar_mask, mem_offset, n_aux = self.embed_prefix(observation)
        ...
        sq_err = jnp.square(v_t - u_t)  # [b, ah, ad]
        masked_sq_err = sq_err * postfix_mask[:, :, None]  # zero out prefix
        # Per-sample loss: average over postfix tokens only
        n_postfix = jnp.sum(postfix_mask, axis=1, keepdims=True).clip(min=1)  # [b, 1]
        action_loss = jnp.sum(masked_sq_err, axis=[1, 2]) / (n_postfix[:, 0] * ad)  # [b]
    ...
    def sample_actions(
        ...
        *,
        ...
        x_0 = jnp.where(rtc_prefix_mask[:, :, None], action_prefix, x_0)  # Ensure exact prefix is returned
        return x_0
